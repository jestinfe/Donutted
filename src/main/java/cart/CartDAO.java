...
public class CartDAO {
	...
	public void insertCartItem(CartItemDTO ciDTO) throws SQLException {
		...
		try {
			...
			insertCart
			.append("insert into cart_item(CART_ITEM_ID, CART_ID, PRODUCT_ID, QUANTITY, ADDED_AT)")
	...
	public List<CartItemDTO> selectAllCartItem(int cartId)throws SQLException{
		...
		try {
			...
			selectCart
			.append("select p.product_id, p.THUMBNAIL_URL, p.name, ci.quantity, p.price ")
			.append(" from cart_item ci ")
			.append(" join  product p ON ci.product_id = p.product_id ")
